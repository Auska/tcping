...
SampleAnalysis analyse(const IConfig& cfg, FDuration* first, FDuration* last) {
  if (!cfg.benchmarkNoAnalysis()) {
    ...
    return SampleAnalysis{
        CATCH_MOVE(samples), Estimate<FDuration>{mean, mean, mean, 0.0},
...
static sample resample(URng& rng, unsigned int resamples, double const* first,
                       double const* last, Estimator& estimator) {
...
static double outlier_variance(Estimate<double> mean, Estimate<double> stddev,
                               int n) {
...
Estimate<double> bootstrap(double confidence_level, double* first, double* last,
  ...
  double prob_n = static_cast<double>(
                      std::count_if(resample.begin(), resample.end(),
...
bootstrap_analysis analyse_samples(double confidence_level,
                                   unsigned int n_resamples, double* first,
                                   double* last) {
...
Config::Config(ConfigData const& data) : m_data(data) {
  ...
  if (m_data.reporterSpecifications.empty()) {
    ...
    CATCH_ENFORCE(parsed, "Cannot parse the provided default reporter spec: '"
                              << default_spec << '\'');
...
std::vector<PathFilter> const& Config::getPathFilters() const {
  return m_data.pathFilters;
}
bool Config::useNewFilterBehaviour() const {
  return m_data.useNewPathFilteringBehaviour;
}
...
std::string const& Config::getExitGuardFilePath() const {
  return m_data.prematureExitGuardFilePath;
}
...
void Config::readBazelEnvVars() {
  ...
  if (bazelRandomSeed) {
    ...
    if (!parsedSeed) {
      ...
      Catch::cerr() << "Warning: could not parse 'TEST_RANDOM_SEED' ('"
...
ScopedMessage::~ScopedMessage() {
  if (!m_moved) {
    IResultCapture::popScopedMessage(m_messageId);
  }
}

Capturer::Capturer(StringRef macroName, SourceLineInfo const& lineInfo,
                   ResultWas::OfType resultType, StringRef names, bool isScoped)
    : m_isScoped(isScoped) {
...
constexpr TestCaseProperties operator|(TestCaseProperties lhs,
...
Detail::unique_ptr<TestCaseInfo> makeTestCaseInfo(
    StringRef _className, NameAndTags const& nameAndTags,
...
bool GeneratorUntypedBase::isFinite() const {
  return true;
}
...
TestCaseStats::TestCaseStats(TestCaseInfo const& _testInfo,
                             Totals const& _totals, std::string&& _stdOut,
                             std::string&& _stdErr, bool _aborting)
    : testInfo(&_testInfo),
      totals(_totals),
...
Clara::Parser makeCommandLineParser(ConfigData& config) {
  ...
  auto const setWarning = [&](std::string const& warning) {
    if (warning == "NoAssertions") {
      ...
      config.warnings = static_cast<WarnAbout::What>(
          config.warnings | WarnAbout::InfiniteGenerator);
      return ParserResult::ok(ParseResultType::Matched);
    }
  ...
  auto const setBenchmarkSamples = [&](std::string const& samples) {
    ...
    if (!parsedSamples) {
      return ParserResult::runtimeError("Could not parse '" + samples +
  ...
  auto const setSectionFilter = [&](std::string const& sectionFilter) {
    config.pathFilters.emplace_back(PathFilter::For::Section,
                                    trim(sectionFilter));
    return ParserResult::ok(ParseResultType::Matched);
  };
  auto const setGeneratorFilter = [&](std::string const& generatorFilter) {
    if (generatorFilter != "*") {
      ...
      if (!parsedIndex) {
        return ParserResult::runtimeError(
            "Could not parse '" + generatorFilter + "' as generator index");
      }
    }
    config.useNewPathFilteringBehaviour = true;
    config.pathFilters.emplace_back(PathFilter::For::Generator,
  ...
  auto cli =
      ...
      Opt(accept_many, setSectionFilter,
          "section name")["-c"]["--section"]("specify section to run") |
      Opt(accept_many, setGeneratorFilter,
          "index spec")["-g"]["--generator-index"](
          "specify generator elements to try") |
      Opt(accept_many, setPathFilter,
          "path filter spec")["-p"]["--path-filter"]("qualified path filter") |
      Opt(setVerbosity,
      ...
      Opt(setTestOrder,
          "decl|lex|rand")["--order"]("test case order (defaults to rand)") |
      ...
      Opt(setBenchmarkSamples, "samples")["--benchmark-samples"](
      ...
      Opt(config.prematureExitGuardFilePath,
          "path")["--premature-exit-guard-file"](
          "create a file before running tests and delete it during clean "
          "exit") |
...
void JsonUtils::appendCommaNewline(std::ostream& os, bool& should_comma,
                                   std::uint64_t level) {
  if (should_comma) {
    os << ',';
  }
...
JsonObjectWriter::JsonObjectWriter(std::ostream& os, std::uint64_t indent_level)
...
JsonArrayWriter::JsonArrayWriter(std::ostream& os, std::uint64_t indent_level)
    : m_os{os}, m_indent_level{indent_level} {
  m_os << '[';
}
...
JsonValueWriter::JsonValueWriter(std::ostream& os, std::uint64_t indent_level)
    : m_os{os}, m_indent_level{indent_level} {}
...
MessageInfo::MessageInfo(StringRef _macroName, SourceLineInfo const& _lineInfo,
  ...
  std::string getContents() {
    ...
    while (current_pos > 0) {
      auto read_characters =
          std::fread(buffer, 1, std::min(buffer_size, current_pos), m_file);
...
public:
  FileRedirect()
      : m_originalOut(dup(fileno(stdout))), m_originalErr(dup(fileno(stderr))) {
...
SimplePcg32::result_type SimplePcg32::operator()() {
  ...
  const auto output =
      rotate_right(xorshifted, static_cast<uint32_t>(m_state >> 59u));
...
struct StringStreams {
  ...
  void release(std::size_t index, std::ostream* originalPtr) {
    ...
    Detail::LockGuard _(m_mutex);
    assert(originalPtr == m_streams[index].get() &&
           "Mismatch between release index and stream ptr");
    m_unused.push_back(index);
  }
};

ReusableStringStream::ReusableStringStream() {
  std::tie(m_index, m_oss) = Singleton<StringStreams>::getMutable().add();
}
...
struct GeneratorTracker final : TestCaseTracking::TrackerBase,
  ...
  void close() override {
    ...
    const bool should_wait_for_child = [&]() {
      ...
      size_t childDepth =
          1 + (m_newStyleFilters ? m_allTrackerDepth : m_sectionOnlyDepth);
    ...
    if (should_wait_for_child ||
        (m_runState == CompletedSuccessfully &&
         !m_isFiltered // filtered generators cannot meaningfully move forward,
                       // as they would get past the filter
...
void RunContext::assertionEnded(AssertionResult&& result) {
  ...
  if (result.getResultType() == ResultWas::Ok) {
    ...
    if (result.isOk()) {
    } else if (m_activeTestCase->getTestCaseInfo()
                   .okToFail()) // Read from a shared state established before
                                // the threads could start, this is fine
  ...
  Detail::LockGuard lock(m_assertionMutex);
  {
    ...
    updateTotalsFromAtomics();
    m_reporter->assertionEnded(
        AssertionStats(result, msgHolder.getMessages(), m_totals));
  }
...
IGeneratorTracker* RunContext::createGeneratorTracker(
    StringRef generatorName, SourceLineInfo lineInfo,
...
void RunContext::handleFatalErrorCondition(StringRef message) {
  ...
  // Since this is a fatal error, we have not had and won't have the opportunity
  // to destruct them properly
...
void RunContext::assertionPassedFastPath(SourceLineInfo lineInfo) {
  // We want to save the line info for better experience with unexpected
  // assertions
  Detail::g_lastKnownLineInfo = lineInfo;
  ++m_atomicAssertionCount.passed;
  Detail::g_lastAssertionPassed = true;
  ...
  CATCH_CATCH_ALL {
    ...
    if (m_shouldReportUnexpected) {
      ...
      handleUnexpectedInflightException(
          makeDummyAssertionInfo(), translateActiveException(), dummyReaction);
    }
  }
  updateTotalsFromAtomics();
...
void RunContext::handleUnexpectedInflightException(
    AssertionInfo const& info, std::string&& message,
    AssertionReaction& reaction) {
  Detail::g_lastKnownLineInfo = info.lineInfo;
...
AssertionInfo RunContext::makeDummyAssertionInfo() {
  const bool testCaseJustStarted = Detail::g_lastKnownLineInfo ==
                                   m_activeTestCase->getTestCaseInfo().lineInfo;

  return AssertionInfo{testCaseJustStarted ? "TEST_CASE"_sr : StringRef(),
...
Section::Section(SectionInfo&& info)
    : m_info(CATCH_MOVE(info)),
      m_sectionIncluded(getResultCapture().sectionStarted(
          m_info.name, m_info.lineInfo, m_assertions)) {
...
Section::Section(SourceLineInfo const& _lineInfo, StringRef _name,
                 const char* const)
...
static bool matchTest(TestCaseHandle const& testCase, TestSpec const& testSpec,
...
TrackerBase::TrackerBase(NameAndLocation&& nameAndLocation, TrackerContext& ctx,
                         ITracker* parent)
...
bool SectionTracker::isComplete() const {
  ...
  if (filterIndex < m_filterRef->size()) {
    ...
    if (m_newStyleFilters &&
        (*m_filterRef)[filterIndex].type != PathFilter::For::Section) {
      return true;
    }
    // 2) Both style filters must match the trimmed name exactly
    if (m_trimmed_name != StringRef((*m_filterRef)[filterIndex].filter)) {
      return true;
    }
  }
...
SectionTracker& SectionTracker::acquire(
    TrackerContext& ctx, NameAndLocationRef const& nameAndLocation) {
...
AutoReg::AutoReg(Detail::unique_ptr<ITestInvoker> invoker,
                 SourceLineInfo const& lineInfo, StringRef classOrMethod,
...
void AnsiSkippingString::preprocessString() {
  for (auto it = m_string.begin(); it != m_string.end();) {
    ...
    while (it != m_string.end() && *it == '\033' && it + 1 != m_string.end() &&
           *(it + 1) == '[') {
      auto cursor = it + 2;
...
std::string AnsiSkippingString::substring(const_iterator begin,
  ...
  // 0xff back to 'm'
  auto str = std::string(begin == this->begin() ? m_string.begin() : begin.m_it,
                         end.m_it);
...
static bool isBoundary(AnsiSkippingString const& line,
  ...
  return it == line.end() ||
         ...
         isBreakableBefore(*it) || isBreakableAfter(*it.oneBefore());
}
...
void Column::const_iterator::calcLength() {
  ...
  while (m_parsedTo != current_line.end() && lineLength < maxLineLength &&
         *m_parsedTo != '\n') {
    ++m_parsedTo;
    ++lineLength;
  }
...
void hexEscapeChar(std::ostream& os, unsigned char c) {
  ...
  os << "\\x"_sr << std::uppercase << std::hex << std::setfill('0')
...
std::string WithinRelMatcher::describe() const {
  Catch::ReusableStringStream sstr;
  sstr << "and " << ::Catch::Detail::stringify(m_target) << " are within "
...
ConsoleReporter::ConsoleReporter(ReporterConfig&& config)
    : StreamingReporterBase(CATCH_MOVE(config)),
      m_tablePrinter(Detail::make_unique<TablePrinter>(
          m_stream, [&config]() -> std::vector<ColumnInfo> {
            if (config.fullConfig()->benchmarkNoAnalysis()) {
              ...
              return {
                  ...
                  {"iterations   low mean   low std dev", 14,
                   Justification::Right},
                  {"est run time high mean  high std dev", 14,
                   Justification::Right}};
            }
          }())) {
...
void defaultListListeners(
    std::ostream& out, std::vector<ListenerDescription> const& descriptions) {
...
void defaultListTags(std::ostream& out, std::vector<TagInfo> const& tags,
                     bool isFiltered) {
  if (isFiltered) {
  ...
  if (!tags.empty()) {
    const auto maxTagCount =
        std::max_element(tags.begin(), tags.end(),
...
void defaultListTests(std::ostream& out, ColourImpl* streamColour,
                      std::vector<TestCaseHandle> const& tests, bool isFiltered,
                      Verbosity verbosity) {
...
void printSummaryRow(std::ostream& stream, ColourImpl& colour, StringRef label,
...
void printTestRunTotals(std::ostream& stream, ColourImpl& streamColour,
                        Totals const& totals) {
  if (totals.testCases.total() == 0) {
...
void JunitReporter::writeSection(std::string const& className,
  ...
  if (sectionNode.stats.assertions.total() > 0 || !sectionNode.stdOut.empty() ||
...
void SonarQubeReporter::writeTestFile(
    StringRef filename, std::vector<TestCaseNode const*> const& testCaseNodes) {
...
public:
  ...
  TapAssertionPrinter(std::ostream& _stream, AssertionStats const& _stats,
                      std::size_t _counter, ColourImpl* colour_)
      : stream(_stream),
        result(_stats.assertionResult),
        messages(_stats.infoMessages),
        itMessage(_stats.infoMessages.begin()),
