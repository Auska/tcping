...
TEST_CASE("PortStatistics - recordHostAttempt", "[statistics]") {
    ...
    SECTION("Record single attempt") {
        portStats.recordHostAttempt("127.0.0.1", 80, true, 10.0, ConnectionState::Success);
    ...
    SECTION("Record multiple hosts same port") {
        portStats.recordHostAttempt("192.168.1.1", 80, true, 10.0, ConnectionState::Success);
        portStats.recordHostAttempt("192.168.1.2", 80, true, 15.0, ConnectionState::Success);
        portStats.recordHostAttempt("192.168.1.3", 80, false, -1.0, ConnectionState::Timeout);
    ...
    SECTION("Record same host multiple ports") {
        portStats.recordHostAttempt("127.0.0.1", 80, true, 10.0, ConnectionState::Success);
        portStats.recordHostAttempt("127.0.0.1", 443, true, 15.0, ConnectionState::Success);
        portStats.recordHostAttempt("127.0.0.1", 22, false, -1.0, ConnectionState::Refused);
...
TEST_CASE("ConnectionState enum", "[statistics]") {
    SECTION("Enum values exist") {
        REQUIRE(static_cast<int>(ConnectionState::Success) != static_cast<int>(ConnectionState::Timeout));
        REQUIRE(static_cast<int>(ConnectionState::Timeout) != static_cast<int>(ConnectionState::Refused));
        REQUIRE(static_cast<int>(ConnectionState::Refused) != static_cast<int>(ConnectionState::Unreachable));
        REQUIRE(static_cast<int>(ConnectionState::Unreachable) != static_cast<int>(ConnectionState::DnsFailure));
    }
}
