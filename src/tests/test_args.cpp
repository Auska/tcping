...
TEST_CASE("ArgumentParser - basic argument parsing", "[args]") {
    SECTION("Minimum required arguments") {
        ...
        Config config;
        REQUIRE(ArgumentParser::parseArguments(3, const_cast<char**>(argv), config));
    ...
    SECTION("Missing arguments") {
        ...
        Config config;
        REQUIRE(!ArgumentParser::parseArguments(2, const_cast<char**>(argv), config));
    }

    SECTION("Too few arguments") {
        const char* argv[] = {"tcping"};
        Config config;
        REQUIRE(!ArgumentParser::parseArguments(1, const_cast<char**>(argv), config));
    }
}
...
TEST_CASE("ArgumentParser - optional arguments", "[args]") {
    SECTION("Verbose flag") {
        ...
        Config config;
        REQUIRE(ArgumentParser::parseArguments(4, const_cast<char**>(argv), config));
        REQUIRE(config.verbose);
    }
    ...
    SECTION("Interval option") {
        ...
        Config config;
        REQUIRE(ArgumentParser::parseArguments(5, const_cast<char**>(argv), config));
...
TEST_CASE("ArgumentParser - combined options", "[args]") {
    SECTION("Multiple options together") {
        const char* argv[] = {"tcping", "127.0.0.1", "80", "-i", "2", "-t", "1000", "-c", "5", "-v", "-a"};
        Config config;
        REQUIRE(ArgumentParser::parseArguments(11, const_cast<char**>(argv), config));
