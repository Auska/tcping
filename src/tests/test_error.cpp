...
#ifdef _WIN32
        REQUIRE(getConnectionState(WSAEHOSTUNREACH) == ConnectionState::Unreachable);
#else
...
#ifdef _WIN32
        REQUIRE(getConnectionState(WSAECONNABORTED) == ConnectionState::ConnectionAborted);
#else
        REQUIRE(getConnectionState(ECONNABORTED) == ConnectionState::ConnectionAborted);
#endif
    }

    SECTION("Connection reset error") {
#ifdef _WIN32
        REQUIRE(getConnectionState(WSAECONNRESET) == ConnectionState::ConnectionReset);
#else
...
TEST_CASE("getDetailedErrorDescription - returns non-empty strings", "[error]") {
...
TEST_CASE("getConnectionStateString - returns correct strings", "[error]") {
    ...
    SECTION("Refused state") {
        REQUIRE(getConnectionStateString(ConnectionState::Refused) == "Connection refused");
    }

    SECTION("Unreachable state") {
        REQUIRE(getConnectionStateString(ConnectionState::Unreachable) == "Host unreachable");
    }

    SECTION("Network down state") {
        REQUIRE(getConnectionStateString(ConnectionState::NetworkDown) == "Network down");
    }

    SECTION("DNS failure state") {
        REQUIRE(getConnectionStateString(ConnectionState::DnsFailure) == "DNS resolution failed");
    }

    SECTION("Connection reset state") {
        REQUIRE(getConnectionStateString(ConnectionState::ConnectionReset) == "Connection reset");
    }

    SECTION("Connection aborted state") {
        REQUIRE(getConnectionStateString(ConnectionState::ConnectionAborted) == "Connection aborted");
    }

    SECTION("Address in use state") {
        REQUIRE(getConnectionStateString(ConnectionState::AddressInUse) == "Address in use");
    }

    SECTION("Access denied state") {
        REQUIRE(getConnectionStateString(ConnectionState::AccessDenied) == "Access denied");
    }

    SECTION("Unknown state") {
        REQUIRE(getConnectionStateString(ConnectionState::Unknown) == "Unknown error");
    }
}
